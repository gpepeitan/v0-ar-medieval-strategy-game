...
export function DiplomacyPanel() {
  ...
  const factions = Array.from(game.factions.values())
  const playerFaction = game.factions.get(playerFactionId)
  const otherFactions = factions.filter(f => f.id !== playerFactionId && !f.isDefeated)

  const getRelation = (factionId: string) =>
    playerFaction?.relations.find(r => r.targetId === factionId)
  ...
  const getMilitaryStrength = (faction: Faction) => {
    let total = 0
    for (const armyId of faction.armies) {
      const army = game.armies.get(armyId)
      if (army) total += army.units.reduce((s, u) => s + u.count, 0)
    }
    return total
  }
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">{otherFactions.length} factions in play</p>

        <div className="space-y-3">
          {otherFactions.map(faction => {
            const config = FACTION_CONFIG[faction.id]
            const relation = getRelation(faction.id)
            const relationValue = relation?.value ?? 0
            const isAtWar = relation?.status === 'war'
            const hasAlliance = relation?.treaties.some(t => t.type === 'alliance' && t.isActive)
            const hasTrade = relation?.treaties.some(t => t.type === 'trade_agreement' && t.isActive)

            return (
              <div key={faction.id} className="p-3 rounded-lg border border-border hover:border-primary/50 transition-colors">
                ...
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  {isAtWar && (
                    <Badge variant="destructive" className="text-xs">
                      <Swords className="h-3 w-3 mr-1" />At War
                    </Badge>
                  )}
                  {hasAlliance && (
                    <Badge variant="default" className="text-xs bg-green-600">
                      <Handshake className="h-3 w-3 mr-1" />Allied
                    </Badge>
                  )}
                  {hasTrade && (
                ...
                <div className="text-xs text-muted-foreground mb-3">
                  ...
                  <span>Strength: {getMilitaryStrength(faction)}</span>
                </div>
